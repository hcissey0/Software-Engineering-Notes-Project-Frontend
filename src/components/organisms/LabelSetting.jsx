...
const LabelSetting = ({label}) => {
  ...
  return (
    <li ref={dropdownRef} className='mb-2 hover:bg-gray-100 cursor-pointer p-1 items-center flex justify-between'>
        <Badge rounded text={label.title} color={label.color}/>
        <IconDots onClick={()=>{setOpen(!open)}}/>
        
        {open && 
        
        <div className='z-10 bg-white divide-y divide-gray-100 absolute left-56 rounded-md shadow-lg w-44 dark:bg-gray-700' style={{top: `${rect.top}px`}}>
            <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
           {
            colors.map((color, index)=>
                <li className='flex items-center px-4 py-2 gap-2 hover:bg-gray-100 dark:hover:bg-gray-600'>
                    <div className={`w-5 aspect-square rounded-full bg-${color}-100`}>
                    </div>
