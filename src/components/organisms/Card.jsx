...
const Card = ({ note, list=false }) => {
  return (
    <div className={` ${list ? 'w-full': 'w-64'} transition-all max-w-full p-4 bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-xl hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700`}>
      ...
      <div className='flex justify-between'>
        ...
        <div>
          <button
          onClick={() => console.log('clicked',note.title)}
          id={note.id}
          data-dropdown-toggle={'dropdown' + note.id}
          className="inline-flex items-center p-2 text-sm font-medium text-center text-gray-900 bg-inherit hover:bg-inherit rounded-lg focus:ring-0 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600"
          type="button">
          ...
          className="z-50 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
              <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby={note.id}>
                <li>
